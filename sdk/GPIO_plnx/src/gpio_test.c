...
void int_delay_meas(void *uio_m, int uio_fd);
void LED_Btn_Test(void *uio_m, int uio_fd);
...
int main()
{
    ...
    writeGPIOPort(uio_m, GPIO_DATA, 0);				// all outputs 0
	...
	int_delay_meas(uio_m, uio_fd);

	//shut down test system
...
//set out0, which is forwarded to input in0
//when interrupt is received and handled, then set out1
// wait 100u, then reset. Repeat every 10 ms
// GPIOs have to be initialized before calling this procedure
void int_delay_meas(void *uio_m, int uio_fd){
	int stop_flag = 1;
    int reenable_int = 1;
    int ires = 0;
    ...
    while(stop_flag){
	    writeGPIOPort(uio_m, GPIO_DATA, 1);				// set out0
		ires = pread(uio_fd, &evt_count, 0x04, 0);		// wait for int
		actIStat = readGPIOPort(uio_m, ISR);			// toggle ISR bits to '0'
		writeGPIOPort(uio_m, ISR, actIStat);
		write(uio_fd, &reenable_int, sizeof(int));
	    writeGPIOPort(uio_m, GPIO_DATA, 3);				// set out1

	    usleep(100);

	    writeGPIOPort(uio_m, GPIO_DATA, 2);				// set out0
		ires = pread(uio_fd, &evt_count, 0x04, 0);		// wait for int
		actIStat = readGPIOPort(uio_m, ISR);			// toggle ISR bits to '0'
		writeGPIOPort(uio_m, ISR, actIStat);
		write(uio_fd, &reenable_int, sizeof(int));
	    writeGPIOPort(uio_m, GPIO_DATA, 0);				// set out1
    ...
    while(wcond){
    	...
    	int ret = 1;
   		if (ret>0){
   			...
   			ires = pread(uio_fd, &evt_count, 0x04, 0);
